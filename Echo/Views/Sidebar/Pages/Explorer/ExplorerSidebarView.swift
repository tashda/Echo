...
struct ExplorerSidebarView: View {
        ...
        ScrollViewReader { proxy in
            ZStack(alignment: .center) {
                VStack(spacing: 0) {
                    ...
                    .onChange(of: sessions.map { $0.connection.id }) {
                    ...
                    .onChange(of: selectedSession?.selectedDatabaseName ?? "") {
    ...
    private var bottomControls: some View {
        if let session = selectedSession,
           ...
           let database = selectedDatabase(in: structure, for: session) {
            let controlHeight = ExplorerSidebarConstants.bottomControlHeight

            VStack(spacing: 0) {
                HStack(spacing: 10) {
                    SearchField(text: $searchText, controlHeight: controlHeight)
                    if database.schemas.count > 1 {
                        SchemaPicker(
                            selection: $selectedSchemaName,
                            database: database,
...
private enum ExplorerSidebarConstants {
    ...
    static let bottomControlHeight: CGFloat = 26
}
...
private struct SearchField: View {
    ...
    private var verticalPadding: CGFloat { max((controlHeight - 18) / 2, 3) }
...
private struct SchemaPicker: View {
    ...
    private var displayTitle: String {
        guard let selected = selection,
              let schema = database.schemas.first(where: { $0.name == selected }) else {
            return "All Schemas"
        }
        return schema.name
    }
    ...
    var body: some View {
        Menu {
            schemaMenuButton(title: "All Schemas", value: nil, isSelected: selection == nil)
            Divider()
            ForEach(database.schemas, id: \.name) { schema in
                schemaMenuButton(
                    title: "\(schema.name) (\(schema.objects.count))",
                    value: schema.name,
                    isSelected: selection == schema.name
                )
            }
        } label: {
            HStack(spacing: 8) {
    ...
    private func schemaMenuButton(title: String, value: String?, isSelected: Bool) -> some View {
        Button {
            selection = value
        } label: {
            HStack {
