...
private struct ClipboardHistoryRow: View {
    ...
    private var metadataSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 8) {
                if let server = entry.metadata.serverName, !server.isEmpty {
                    metadataBadge(icon: "server.rack", text: server)
                }
                if let database = entry.metadata.databaseName, !database.isEmpty {
                    metadataBadge(icon: "cylinder", text: database)
                }
                if let object = entry.metadata.objectName, !object.isEmpty {
