...
struct SettingsView: View {
    ...
    private var activeSection: SettingsSection { selectedSection ?? .appearance }
        ...
        NavigationSplitView(columnVisibility: $columnVisibility, preferredCompactColumn: $preferredColumn) {
            ...
            NavigationStack {
                detailView(for: activeSection)
                    .navigationTitle(activeSection.displayName)
            }
...
struct AppearanceSettingsView: View {
    ...
    private var paletteGrid: some View {
        VStack(alignment: .leading, spacing: 28) {
            if !lightPalettes.isEmpty {
                paletteSection(title: "Light Palettes", palettes: lightPalettes)
            }

            if !darkPalettes.isEmpty {
                paletteSection(title: "Dark Palettes", palettes: darkPalettes)
            }
        }
    }
    ...
    private var sqlEditorSection: some View {
        Section {
            ...
            Text("This palette is used by default for all projects. Project-specific palettes can be set in the project manager.")
        }
    }
    ...
    private var lightPalettes: [SQLEditorPalette] {
        availablePalettes.filter { !$0.isDark }
    }

    private var darkPalettes: [SQLEditorPalette] {
        availablePalettes.filter { $0.isDark }
    }

    private var paletteColumns: [GridItem] {
        [GridItem(.adaptive(minimum: 240, maximum: 260), spacing: 18)]
    }
    ...
    private var paletteSelectionBinding: Binding<String> {
        Binding(
            get: { appModel.globalSettings.defaultEditorPaletteID },
            set: { newValue in
                guard let palette = availablePalettes.first(where: { $0.id == newValue }) else { return }
    ...
    @ViewBuilder
    private func paletteSection(title: String, palettes: [SQLEditorPalette]) -> some View {
    ...
    private var defaultPaletteID: String {
        appModel.globalSettings.defaultEditorPaletteID
    }

    private var selectedPalette: SQLEditorPalette {
        appModel.globalSettings.palette(withID: defaultPaletteID) ?? SQLEditorPalette.aurora
    }
    ...
    private func selectPalette(_ palette: SQLEditorPalette) {
        Task {
            isPersistingPalette = true
            await appModel.setDefaultEditorPalette(to: palette.id)
    ...
    private var storageLocationSection: some View {
        Section("Storage Location") {
            VStack(alignment: .leading, spacing: 6) {
                Text("~/Library/Application Support/Echo/ClipboardHistory")
                    .font(.system(size: 12, weight: .semibold))
                    .textSelection(.enabled)

                Text("Use Finder's Go to Folder to inspect or remove files manually if needed.")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
            .padding(.vertical, 4)
        ...
        Button(action: action) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 18, style: .continuous)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 18, style: .continuous)
                            .strokeBorder(isSelected ? Color.accentColor : theme.previewStyle.stroke, lineWidth: isSelected ? 3 : 1)
                    )
                    .shadow(color: theme.previewStyle.shadow.opacity(isSelected ? 0.45 : 0.25), radius: isSelected ? 14 : 8, y: isSelected ? 8 : 4)
            )
        }
        .buttonStyle(.plain)
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 16, style: .continuous)
                        .stroke(style.chrome.opacity(0.15), lineWidth: 1)
                )
            ...
            if isActive {
                LinearGradient(colors: [style.accent.primary.opacity(0.25), .clear], startPoint: .topLeading, endPoint: .bottomTrailing)
    ...
    var previewStyle: ThemePreviewStyle {
        ...
        case .light:
            return ThemePreviewStyle(
                gradient: LinearGradient(colors: [Color(red: 0.98, green: 0.99, blue: 1.0), Color(red: 0.93, green: 0.95, blue: 0.99)], startPoint: .topLeading, endPoint: .bottomTrailing),
        ...
        case .dark:
            return ThemePreviewStyle(
                gradient: LinearGradient(colors: [Color(red: 0.08, green: 0.09, blue: 0.13), Color(red: 0.17, green: 0.19, blue: 0.24)], startPoint: .top, endPoint: .bottom),
        ...
        case .system:
            return ThemePreviewStyle(
                gradient: LinearGradient(colors: [Color(red: 0.11, green: 0.13, blue: 0.17), Color(red: 0.98, green: 0.98, blue: 1.0)], startPoint: .leading, endPoint: .trailing),
...
private struct PaletteCard: View {
    ...
    private var foreground: Color {
        palette.isDark ? .white : Color.black.opacity(0.85)
    }
    ...
    private var cardBackground: Color {
        palette.isDark ? Color(red: 0.12, green: 0.14, blue: 0.18) : Color(red: 0.98, green: 0.99, blue: 1.0)
    }
    ...
    var body: some View {
        Button(action: action) {
            VStack(alignment: .leading, spacing: 14) {
                ZStack(alignment: .topLeading) {
                    PalettePreview(palette: palette)
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 14, style: .continuous)
                                .stroke(palette.isDark ? Color.white.opacity(0.06) : Color.black.opacity(0.04), lineWidth: 1)
                        )
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 12, style: .continuous)
                            .fill(palette.isDark ? Color.black.opacity(0.6) : Color.white.opacity(0.85))
                    )
                    .padding(12)
                }
                ...
                HStack(spacing: 8) {
                    ForEach(Array(palette.showcaseColors.enumerated()), id: \.offset) { _, swatch in
                        Capsule(style: .continuous)
                            ...
                            .overlay(
                                Capsule(style: .continuous)
                                    .stroke(Color.white.opacity(palette.isDark ? 0.35 : 0.15), lineWidth: 0.8)
                            )
                    }
                    Spacer(minLength: 0)
                }
                ...
                if isSelected {
                    Label("Current default", systemImage: "checkmark.circle.fill")
                        .font(.footnote)
                        .fontWeight(.semibold)
                        .foregroundStyle(.tint)
            ...
            .shadow(color: cardShadow, radius: isSelected ? 16 : 10, y: isSelected ? 10 : 6)
            .overlay(
                RoundedRectangle(cornerRadius: 20, style: .continuous)
                    .strokeBorder(Color.white.opacity(palette.isDark ? 0.08 : 0.0))
            )
        }
        .buttonStyle(.plain)
