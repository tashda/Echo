...
final class AppModel: ObservableObject {
    ...
    @Published var searchSidebarCache: SearchSidebarCache = SearchSidebarCache()
    ...
    private func normalizeEditorPreferences() async {
        ...
        if globalSettings.defaultEditorPaletteID.isEmpty || globalSettings.palette(withID: globalSettings.defaultEditorPaletteID) == nil {
            if let legacy = globalSettings.palette(withID: globalSettings.defaultEditorTheme) {
                globalSettings.defaultEditorPaletteID = legacy.id
            } else {
                globalSettings.defaultEditorPaletteID = SQLEditorPalette.aurora.id
            }
    ...
    func setDefaultEditorPalette(to paletteID: String) async {
        globalSettings.defaultEditorPaletteID = paletteID
        await persistGlobalSettings()
    }
    ...
    func deleteCustomPalette(withID id: String) async {
        ...
        if globalSettings.defaultEditorPaletteID == id {
            globalSettings.defaultEditorPaletteID = SQLEditorPalette.aurora.id
        }
