...
actor SQLiteSession: DatabaseSession {
        ...
        return try withSQLiteStatement(connection: conn, sql: sql) { statement in
            ...
            while true {
                ...
                case SQLITE_DONE:
                    ...
                    return QueryResultSet(columns: update.columns, rows: rows)
                default:
    ...
    func listTablesAndViews(schema: String?) async throws -> [SchemaObjectInfo] {
        ...
        return try withSQLiteStatement(connection: conn, sql: sql) { statement in
            ...
            while sqlite3_step(statement) == SQLITE_ROW {
                ...
                guard let typePtr = sqlite3_column_text(statement, 1) else { continue }
                let typeString = String(cString: typePtr)
                guard let objectType = SchemaObjectInfo.ObjectType(sqliteType: typeString) else { continue }
                objects.append(SchemaObjectInfo(name: name, schema: databaseName, type: objectType))
            }
            return objects
        }
    }
    ...
    func getObjectDefinition(objectName: String, schemaName: String, objectType: SchemaObjectInfo.ObjectType) async throws -> String {
        let conn = try requireConnection()
        let databaseName = normalizedDatabaseName(schemaName)
        let typeString = objectType.sqliteMasterType
    ...
    func getTableStructureDetails(schema: String, table: String) async throws -> TableStructureDetails {
        ...
        let indexes = try fetchIndexes(schema: schema, table: table)
        let uniqueConstraints = indexes.filter(\.isUnique).map {
            TableStructureDetails.UniqueConstraint(name: $0.name, columns: $0.columns.map(\.name))
        }
        let foreignKeys = try fetchForeignKeys(schema: schema, table: table)
    ...
    private func fetchIndexes(schema: String, table: String) throws -> [TableStructureDetails.Index] {
        let conn = try requireConnection()
        let databaseName = normalizedDatabaseName(schema)
        let pragma = "PRAGMA \(databaseName).index_list('\(escapeSingleQuotes(table))');"
        return try withSQLiteStatement(connection: conn, sql: pragma) { statement in
            var collected: [TableStructureDetails.Index] = []
            while sqlite3_step(statement) == SQLITE_ROW {
                guard
    ...
    private func fetchIndexColumns(databaseName: String, indexName: String) throws -> [TableStructureDetails.Index.Column] {
        let conn = try requireConnection()
        ...
        return try withSQLiteStatement(connection: conn, sql: pragma) { statement in
            ...
            while sqlite3_step(statement) == SQLITE_ROW {
                ...
                let name = String(cString: namePtr)
                let sortOrder = sqlite3_column_int(statement, 3) == 0 ? TableStructureDetails.Index.Column.SortOrder.ascending : .descending
                columns.append(TableStructureDetails.Index.Column(name: name, position: position, sortOrder: sortOrder))
            }
    ...
    private func fetchForeignKeys(schema: String, table: String) throws -> [TableStructureDetails.ForeignKey] {
        let conn = try requireConnection()
        let databaseName = normalizedDatabaseName(schema)
            ...
            for key in grouped.keys.sorted() {
                ...
                foreignKeys.append(
                    TableStructureDetails.ForeignKey(
                        name: "fk_\(table)_\(key)",
                        columns: entry.columns,
                        referencedSchema: normalizedDatabaseName(schema),
...
private extension SchemaObjectInfo.ObjectType {
    ...
    var sqliteMasterType: String? {
        switch self {
        case .table: return "table"
        case .view: return "view"
        case .trigger: return "trigger"
        case .materializedView, .function: return nil
        }
    }
}
