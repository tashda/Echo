...
struct DatabaseExplorerSidebarView: View {
    ...
    @StateObject internal var viewModel = DatabaseExplorerSidebarViewModel()
    ...
    private var sessions: [ConnectionSession] { workspaceSessionStore.sessionManager.sessions }
                ...
                VStack(spacing: 0) {
                    ...
                    .onChange(of: selectedConnectionID) { _, newValue in
                        guard let id = newValue,
                              let session = workspaceSessionStore.sessionManager.sessionForConnection(id) else { return }
    ...
    private func refreshSelectedSessionStructure() async {
        ...
        await workspaceSessionStore.refreshDatabaseStructure(for: session.id)
    }
