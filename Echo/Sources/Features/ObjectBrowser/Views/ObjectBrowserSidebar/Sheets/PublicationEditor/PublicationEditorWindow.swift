...
struct PublicationEditorWindow: Scene {
    static let sceneID = "publication-editor"
    private let coordinator = AppDirector.shared

    var body: some Scene {
        WindowGroup(id: Self.sceneID, for: PublicationEditorWindowValue.self) { $value in
            if let value {
                PublicationEditorWindowContent(windowValue: value)
...
private struct PublicationEditorWindowContent: View {
    let windowValue: PublicationEditorWindowValue
    @Environment(EnvironmentState.self) private var environmentState
    @Environment(AppearanceStore.self) private var appearanceStore
    @Environment(ActivityEngine.self) private var activityEngine
    @Environment(\.dismiss) private var dismiss

    private var viewModel: PublicationEditorViewModel? {
        environmentState.publicationEditorViewModels[windowValue]
    }
    ...
    var body: some View {
        if let viewModel, let session = connectionSession {
            PublicationEditorView(viewModel: viewModel, session: session) {
                dismiss()
            }
            .onAppear {
