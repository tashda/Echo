...
struct SubscriptionEditorWindow: Scene {
    static let sceneID = "subscription-editor"
    private let coordinator = AppDirector.shared

    var body: some Scene {
        WindowGroup(id: Self.sceneID, for: SubscriptionEditorWindowValue.self) { $value in
            if let value {
                SubscriptionEditorWindowContent(windowValue: value)
...
private struct SubscriptionEditorWindowContent: View {
    let windowValue: SubscriptionEditorWindowValue
    @Environment(EnvironmentState.self) private var environmentState
    @Environment(AppearanceStore.self) private var appearanceStore
    @Environment(ActivityEngine.self) private var activityEngine
    @Environment(\.dismiss) private var dismiss

    private var viewModel: SubscriptionEditorViewModel? {
        environmentState.subscriptionEditorViewModels[windowValue]
    }
    ...
    var body: some View {
        if let viewModel, let session = connectionSession {
            SubscriptionEditorView(viewModel: viewModel, session: session) {
                dismiss()
            }
            .onAppear {
