...
struct FunctionEditorView: View {
    ...
    @State private var selectedPage: FunctionEditorPage? = .general

    private var displayName: String {
        viewModel.functionName.isEmpty ? "New Function" : viewModel.functionName
    }
    ...
    // MARK: - Definition Detail (non-Form)

    private var definitionDetail: some View {
        FunctionEditorDefinitionPage(viewModel: viewModel)
            .overlay {
                if viewModel.isLoading {
                    TabInitializingPlaceholder(
                        icon: "curlybraces",
    ...
    // MARK: - SQL Detail (non-Form)

    private var sqlDetail: some View {
        FunctionEditorSQLPage(viewModel: viewModel)
    }
    ...
    private var pageContent: some View {
        switch selectedPage {
        case .general:
            FunctionEditorGeneralPage(viewModel: viewModel)
        case .parameters:
            FunctionEditorParametersPage(viewModel: viewModel)
        case .options:
            FunctionEditorOptionsPage(viewModel: viewModel)
        case .definition, .sql, nil:
            EmptyView()
        }
    }
