...
struct SequenceEditorWindow: Scene {
    static let sceneID = "sequence-editor"
    private let coordinator = AppDirector.shared

    var body: some Scene {
        WindowGroup(id: Self.sceneID, for: SequenceEditorWindowValue.self) { $value in
            if let value {
                SequenceEditorWindowContent(windowValue: value)
...
private struct SequenceEditorWindowContent: View {
    let windowValue: SequenceEditorWindowValue
    @Environment(EnvironmentState.self) private var environmentState
    @Environment(AppearanceStore.self) private var appearanceStore
    @Environment(ActivityEngine.self) private var activityEngine
    @Environment(\.dismiss) private var dismiss

    private var viewModel: SequenceEditorViewModel? {
        environmentState.sequenceEditorViewModels[windowValue]
    }
    ...
    var body: some View {
        if let viewModel, let session = connectionSession {
            SequenceEditorView(viewModel: viewModel, session: session) {
                dismiss()
            }
            .onAppear {
