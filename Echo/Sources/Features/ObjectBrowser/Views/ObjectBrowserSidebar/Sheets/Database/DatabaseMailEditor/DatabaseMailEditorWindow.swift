...
struct DatabaseMailEditorWindow: Scene {
    static let sceneID = "database-mail-editor"
    private let coordinator = AppDirector.shared

    var body: some Scene {
        WindowGroup(id: Self.sceneID, for: DatabaseMailEditorWindowValue.self) { $value in
            if let value {
                DatabaseMailEditorWindowContent(windowValue: value)
...
private struct DatabaseMailEditorWindowContent: View {
    let windowValue: DatabaseMailEditorWindowValue
    @Environment(EnvironmentState.self) private var environmentState
    @Environment(AppearanceStore.self) private var appearanceStore
    @Environment(ActivityEngine.self) private var activityEngine
    @Environment(\.dismiss) private var dismiss

    private var viewModel: DatabaseMailEditorViewModel? {
        environmentState.databaseMailEditorViewModels[windowValue]
    }
    ...
    var body: some View {
        if let viewModel, let session = connectionSession {
            DatabaseMailEditorView(viewModel: viewModel, session: session) {
                dismiss()
            }
            .onAppear {
