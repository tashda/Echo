...
struct DatabaseEditorWindow: Scene {
    static let sceneID = "database-editor"
    private let coordinator = AppDirector.shared

    var body: some Scene {
        WindowGroup(id: Self.sceneID, for: DatabaseEditorWindowValue.self) { $value in
            if let value {
                DatabaseEditorWindowContent(windowValue: value)
...
private struct DatabaseEditorWindowContent: View {
    let windowValue: DatabaseEditorWindowValue
    ...
    private var viewModel: DatabaseEditorViewModel? {
        environmentState.databaseEditorViewModels[windowValue]
    }
    ...
    var body: some View {
        if let viewModel, let session = connectionSession {
            DatabaseEditorView(viewModel: viewModel, session: session) {
                dismiss()
            }
            .onAppear {
