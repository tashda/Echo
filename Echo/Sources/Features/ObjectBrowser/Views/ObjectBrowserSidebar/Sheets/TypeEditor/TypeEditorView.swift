...
struct TypeEditorView: View {
    ...
    @State private var selectedPage: TypeEditorPage? = .general

    private var displayName: String {
        if viewModel.typeName.isEmpty {
            return "New \(viewModel.typeCategory.title) Type"
        }
    ...
    var body: some View {
        NavigationSplitView {
            List(viewModel.pages, id: \.self, selection: $selectedPage) { page in
                Label(pageTitleFor(page), systemImage: page.icon)
                    .tag(page)
            }
            .listStyle(.sidebar)
    ...
    // MARK: - Page Title Override

    private func pageTitleFor(_ page: TypeEditorPage) -> String {
        if page == .attributes { return viewModel.attributesPageTitle }
        return page.title
    }
    ...
    private var sqlDetail: some View {
        TypeEditorSQLPage(viewModel: viewModel)
    }
    ...
    private var pageContent: some View {
        switch selectedPage {
        case .general:
            TypeEditorGeneralPage(viewModel: viewModel)
        case .attributes:
            TypeEditorAttributesPage(viewModel: viewModel)
        case .sql, nil:
            EmptyView()
        }
    }
