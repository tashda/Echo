...
struct LoginEditorWindow: Scene {
    static let sceneID = "login-editor"
    private let coordinator = AppDirector.shared

    var body: some Scene {
        WindowGroup(id: Self.sceneID, for: LoginEditorWindowValue.self) { $value in
            if let value {
                LoginEditorWindowContent(windowValue: value)
...
private struct LoginEditorWindowContent: View {
    let windowValue: LoginEditorWindowValue
    @Environment(EnvironmentState.self) private var environmentState
    @Environment(AppearanceStore.self) private var appearanceStore
    @Environment(ActivityEngine.self) private var activityEngine
    @Environment(\.dismiss) private var dismiss

    private var viewModel: LoginEditorViewModel? {
        environmentState.loginEditorViewModels[windowValue]
    }
    ...
    var body: some View {
        if let viewModel, let session = connectionSession {
            LoginEditorView(viewModel: viewModel, session: session) {
                dismiss()
            }
            .onAppear {
