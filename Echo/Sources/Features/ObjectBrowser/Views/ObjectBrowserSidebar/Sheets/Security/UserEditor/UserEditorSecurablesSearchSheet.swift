...
struct UserEditorSecurablesSearchSheet: View {
    ...
    private func addSelectedSecurables() {
        for resultID in selectedResults {
            ...
            let permissions = applicablePermissions(for: item)
    ...
    private func applicablePermissions(for item: SearchResultItem) -> [String] {
        switch item.objectKind {
        case .table, .view:
            return ["SELECT", "INSERT", "UPDATE", "DELETE", "REFERENCES",
                    "ALTER", "CONTROL", "TAKE OWNERSHIP", "VIEW DEFINITION",
                    "VIEW CHANGE TRACKING"]
        case .procedure, .function, .inlineTableFunction, .scalarFunction:
            return ["EXECUTE", "ALTER", "CONTROL", "TAKE OWNERSHIP", "VIEW DEFINITION"]
        case nil: // Schema
            return ["ALTER", "CONTROL", "TAKE OWNERSHIP", "CREATE TABLE",
                    "CREATE VIEW", "CREATE PROCEDURE", "CREATE FUNCTION"]
        case .unknown:
            return ["SELECT", "INSERT", "UPDATE", "DELETE", "EXECUTE"]
        }
    }
}
...
enum ObjectTypeFilter: String, CaseIterable, Identifiable {
    case all, tables, views, schemas
