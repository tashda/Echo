...
struct UserEditorWindow: Scene {
    static let sceneID = "user-editor"
    private let coordinator = AppDirector.shared

    var body: some Scene {
        WindowGroup(id: Self.sceneID, for: UserEditorWindowValue.self) { $value in
            if let value {
                UserEditorWindowContent(windowValue: value)
...
private struct UserEditorWindowContent: View {
    let windowValue: UserEditorWindowValue
    @Environment(EnvironmentState.self) private var environmentState
    @Environment(AppearanceStore.self) private var appearanceStore
    @Environment(ActivityEngine.self) private var activityEngine
    @Environment(\.dismiss) private var dismiss

    private var viewModel: UserEditorViewModel? {
        environmentState.userEditorViewModels[windowValue]
    }
    ...
    var body: some View {
        if let viewModel, let session = connectionSession {
            UserEditorView(viewModel: viewModel, session: session) {
                dismiss()
            }
            .onAppear {
