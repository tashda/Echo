...
struct RoleEditorWindow: Scene {
    static let sceneID = "role-editor"
    private let coordinator = AppDirector.shared

    var body: some Scene {
        WindowGroup(id: Self.sceneID, for: RoleEditorWindowValue.self) { $value in
            if let value {
                RoleEditorWindowContent(windowValue: value)
...
private struct RoleEditorWindowContent: View {
    let windowValue: RoleEditorWindowValue
    @Environment(EnvironmentState.self) private var environmentState
    @Environment(AppearanceStore.self) private var appearanceStore
    @Environment(ActivityEngine.self) private var activityEngine
    @Environment(\.dismiss) private var dismiss

    private var viewModel: RoleEditorViewModel? {
        environmentState.roleEditorViewModels[windowValue]
    }
    ...
    var body: some View {
        if let viewModel, let session = connectionSession {
            RoleEditorView(viewModel: viewModel, session: session) {
                dismiss()
            }
            .onAppear {
