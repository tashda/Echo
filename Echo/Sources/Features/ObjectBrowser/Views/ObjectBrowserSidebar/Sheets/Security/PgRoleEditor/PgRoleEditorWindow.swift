...
struct PgRoleEditorWindow: Scene {
    static let sceneID = "pg-role-editor"
    private let coordinator = AppDirector.shared

    var body: some Scene {
        WindowGroup(id: Self.sceneID, for: PgRoleEditorWindowValue.self) { $value in
            if let value {
                PgRoleEditorWindowContent(windowValue: value)
...
private struct PgRoleEditorWindowContent: View {
    let windowValue: PgRoleEditorWindowValue
    @Environment(EnvironmentState.self) private var environmentState
    @Environment(AppearanceStore.self) private var appearanceStore
    @Environment(ActivityEngine.self) private var activityEngine
    @Environment(\.dismiss) private var dismiss

    private var viewModel: PgRoleEditorViewModel? {
        environmentState.pgRoleEditorViewModels[windowValue]
    }
    ...
    var body: some View {
        if let viewModel, let session = connectionSession {
            PgRoleEditorView(viewModel: viewModel, session: session) {
                dismiss()
            }
            .onAppear {
