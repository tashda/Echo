...
struct ConnectMenuCommands: Commands {
        ...
        CommandMenu("Connect") {
            ...
            if hasActiveSessions {
                ForEach(activeSessions, id: \.id) { session in
                    let isPrimary = session.id == environmentState.sessionManager.activeSessionID
    ...
    private func prioritizedSessions(for projectID: UUID?) -> [ConnectionSession] {
        var sessions = environmentState.sessionManager.sortedSessions
        guard !sessions.isEmpty else { return [] }

        if let activeID = environmentState.sessionManager.activeSessionID,
