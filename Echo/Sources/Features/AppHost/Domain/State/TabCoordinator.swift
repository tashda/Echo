...
extension TabCoordinatorDelegate {
    func tabCoordinator(_ manager: TabCoordinator, shouldClose tab: WorkspaceTab) -> Bool { true }
}
...
final class TabCoordinator: ObservableObject {
    ...
    @Published var activeTabId: UUID? {
        didSet {
            ...
            delegate?.tabCoordinator(self, didSetActiveTabID: activeTabId)
        }
    }
    ...
    func insertTab(_ tab: WorkspaceTab, at index: Int, activate shouldActivate: Bool = true, notifyDelegate: Bool = true) {
        ...
        if notifyDelegate {
            delegate?.tabCoordinator(self, didAdd: tab)
        }
    }
    ...
    func removeTab(withID id: UUID) {
        ...
        delegate?.tabCoordinator(self, didRemoveTabID: id)
    }
    ...
    func closeTab(id: UUID) -> Bool {
        ...
        let tab = tabs[index]
        guard delegate?.tabCoordinator(self, shouldClose: tab) ?? true else { return false }
    ...
    func moveTab(id: UUID, to proposedIndex: Int) {
        ...
        delegate?.tabCoordinatorDidReorderTabs(self)
    }
