...
struct WorkspaceToolbarItems: ToolbarContent {
    ...
    internal var activeSession: ConnectionSession? {
        if let connection = navigationStore.navigationState.selectedConnection,
           let session = environmentState.sessionManager.sessionForConnection(connection.id) {
            return session
        }
        return environmentState.sessionManager.activeSession ?? environmentState.sessionManager.activeSessions.first
    }
