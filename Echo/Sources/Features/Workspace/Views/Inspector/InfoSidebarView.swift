...
struct InfoSidebarView: View {
    ...
    @EnvironmentObject private var workspaceSessionStore: WorkspaceSessionStore // Temporary bridge for content and actions
    ...
    var body: some View {
        ...
        .onChange(of: workspaceSessionStore.dataInspectorContent) { _, _ in
    ...
    private var dataInspectorContent: some View {
        if let content = workspaceSessionStore.dataInspectorContent {
            VStack(alignment: .leading, spacing: 16) {
                switch content {
    ...
    private var connectionInspectorContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if let session = workspaceSessionStore.sessionManager.activeSession {
...
private struct InspectorPanelView: View {
    ...
    private func openForeignRecord(with sql: String) {
        workspaceSessionStore.openQueryTab(presetQuery: sql, autoExecute: true)
    }
}
