...
@Observable
final class ResultSpoolConfigCoordinator: ResultSpoolCoordinatorProtocol, @unchecked Sendable {
    private let spoolManager: ResultSpoolCoordinator

    init(spoolManager: ResultSpoolCoordinator) {
    ...
    func updateConfiguration(with settings: GlobalSettings) async {
        ...
        if let path = path, !path.isEmpty {
            ...
            rootDirectory = ResultSpoolCoordinator.defaultRootDirectory()
        }
