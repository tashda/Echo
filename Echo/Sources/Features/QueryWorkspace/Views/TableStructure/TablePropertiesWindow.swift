...
struct TablePropertiesWindow: Scene {
    static let sceneID = "table-properties"
    private let coordinator = AppDirector.shared

    var body: some Scene {
        WindowGroup(id: Self.sceneID, for: TablePropertiesWindowValue.self) { $value in
            if let value {
                TablePropertiesWindowContent(windowValue: value)
...
private struct TablePropertiesWindowContent: View {
    let windowValue: TablePropertiesWindowValue
    ...
    private var viewModel: TablePropertiesViewModel? {
        environmentState.tablePropertiesViewModels[windowValue]
    }
    ...
    var body: some View {
        if let viewModel, let session = connectionSession {
            TablePropertiesView(viewModel: viewModel, session: session) {
                dismiss()
            }
            .onAppear {
