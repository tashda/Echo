...
extension SQLTextView {
    ...
    func applyCompletion(_ suggestion: SQLAutoCompletionSuggestion, response: SQLCompletionResponse) {
        if isStarExpansionSuggestion(suggestion) {
            let context = completionContext
            Task { [weak self] in
                guard let self else { return }
                ...
                await MainActor.run {
                    if self.performCompletionInsertion(suggestion: suggestion,
                                                       replacementRange: response.replacementRange,
                                                       insertion: formatted) != nil {
                        self.undoManager?.setActionName("Expand Columns")
                        self.completionEngine.clearSuppression()
                    }
                }
            }
            return
