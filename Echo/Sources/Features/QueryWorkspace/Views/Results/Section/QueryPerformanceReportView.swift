...
struct QueryPerformanceReportView: View {
            ...
            VStack(alignment: .leading, spacing: SpacingTokens.md) {
                ...
                if let report {
                    ...
                    ContentUnavailableView {
                        ...
                        Text("Run a query to capture timing, batch flow, and resource usage.")
                    }
                }
            }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: SpacingTokens.xs) {
            HStack(alignment: .center, spacing: SpacingTokens.sm) {
                Text(title.isEmpty ? "Untitled Query" : title)
            ...
            HStack(spacing: SpacingTokens.md) {
                if let connectionName = connectionName?.nonEmpty {
                    Label(connectionName, systemImage: "server.rack")
                        .labelStyle(.titleAndIcon)
                        .font(TypographyTokens.caption)
                        .foregroundStyle(ColorTokens.Text.secondary)
                }
                if let databaseName = databaseName?.nonEmpty {
                    Label(databaseName, systemImage: "database")
                        .labelStyle(.titleAndIcon)
                        .font(TypographyTokens.caption)
                        .foregroundStyle(ColorTokens.Text.secondary)
                }
    ...
    private func primaryMetrics(for report: QueryPerformanceTracker.Report) -> [Metric] {
        ...
        items.append(Metric(label: "Initial \(report.initialBatchTarget)", value: EchoFormatters.duration(report.timings.startToInitialBatch)))
