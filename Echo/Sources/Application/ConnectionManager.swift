...
final class ConnectionManager: ObservableObject {
    ...
    func saveConnection(_ connection: SavedConnection, password: String? = nil) async {
        ...
        if let password = password, !password.isEmpty {
            if conn.keychainIdentifier == nil {
                conn.keychainIdentifier = "echo.\(conn.id.uuidString)"
            }
    ...
    func connect(to connection: SavedConnection) async {
        ...
        do {
            ...
            guard connection.credentialSource == .manual else {
                throw DatabaseError.connectionFailed("ConnectionManager supports manual credentials only")
            }

            // Create connection
            guard let factory = makeDatabaseFactory(for: connection.databaseType) else {
                throw DatabaseError.connectionFailed("Unsupported database type: \(connection.databaseType.displayName)")
            }
            ...
            let authentication = DatabaseAuthenticationConfiguration(
                ...
                password: password,
                domain: connection.domain.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? nil : connection.domain.trimmingCharacters(in: .whitespacesAndNewlines)
            )
    ...
    func listTables() async throws -> [String] {
        ...
        let schema: String?
        switch selectedConnection?.databaseType {
        case .postgresql:
            schema = "public"
        default:
            schema = nil
        ...
        do {
            ...
            guard connection.credentialSource == .manual else {
                return ConnectionTestResult(
                    isSuccessful: false,
                    message: "ConnectionManager supports manual credentials only",
