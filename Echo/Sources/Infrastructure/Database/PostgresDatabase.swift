...
final class PostgresSession: DatabaseSession {
    ...
    private func streamQuery(
        ...
        // Build options for future wire-level routing, but for performance we still
        // apply our proven fast-path routing here until postgres-wire implements it.
        ...
        // Map Echo settings to wire options
        ...
        let baselineFetch = {
            let stored = defaults.integer(forKey: ResultStreamingFetchSizeDefaultsKey)
            return stored >= 128 ? stored : 4_096
        }()
        let rampMultiplier = max(1, defaults.integer(forKey: ResultStreamingFetchRampMultiplierDefaultsKey))
        let rampMax = max(256, defaults.integer(forKey: ResultStreamingFetchRampMaxDefaultsKey))

        // Translate mode
        let wireMode: PostgresExecutionMode = {
            switch selectedMode {
        ...
        let wireOptions = PostgresExecutionOptions(
            mode: wireMode,
            cursorThreshold: cursorThreshold,
            fetchBaseline: baselineFetch,
            fetchRampMultiplier: rampMultiplier,
            fetchRampMax: rampMax,
            progressThrottleMs: 120
        )

        // TEMP: Until postgres-wire owns routing, keep Echo’s optimized fast paths.
    ...
    private func streamQueryUsingWireOptions(
        sanitizedSQL: String,
        ...
        // Fetch rows via wire-level options; iterate and reuse the existing ingestion path.
        let rowSequence = try await self.client.simpleQuery(sanitizedSQL, options: options)
        ...
        for try await row in rowSequence {
            ...
            let needsRawPayloadForDeferred = formattingEnabled && (formattingMode == .deferred)

            let shouldFormatRow: Bool
            if !formattingEnabled {
                shouldFormatRow = false
            } else {
            ...
            if shouldFormatRow || needsRawPayloadForDeferred { payloadCells.reserveCapacity(row.count) }
            ...
            } else if needsRawPayloadForDeferred {
            ...
            if totalRowCount < streamingPreviewLimit { continue }
        ...
        return QueryResultSet(columns: resolvedColumns,
                              rows: previewRows,
