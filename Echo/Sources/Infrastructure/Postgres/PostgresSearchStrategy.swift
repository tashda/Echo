...
struct PostgresDatabaseSearchStrategy: DatabaseSearchStrategy {
    ...
    func searchTables(query: String) async throws -> [SearchSidebarResult] {
        ...
        let sql = PostgresSearchSQL.tables(pattern: pattern, limit: DatabaseSearchProvider.QueryConstants.maxNameResults)
    ...
    func searchViews(query: String) async throws -> [SearchSidebarResult] {
        let pattern = PostgresSearchSQL.makeLikePattern(query)
        let sql = PostgresSearchSQL.views(pattern: pattern, limit: DatabaseSearchProvider.QueryConstants.maxNameResults)
        let result = try await session.simpleQuery(sql)
        return result.rows.compactMap { row in
    ...
    func searchMaterializedViews(query: String) async throws -> [SearchSidebarResult] {
        let pattern = PostgresSearchSQL.makeLikePattern(query)
        let sql = PostgresSearchSQL.materializedViews(pattern: pattern, limit: DatabaseSearchProvider.QueryConstants.maxNameResults)
        let result = try await session.simpleQuery(sql)
        return result.rows.compactMap { row in
            guard row.count >= 3, let schema = row[0], let name = row[1] else { return nil }
            let definition = row[2] ?? ""
    ...
    func searchFunctions(query: String) async throws -> [SearchSidebarResult] {
        let pattern = PostgresSearchSQL.makeLikePattern(query)
        let sql = PostgresSearchSQL.functions(pattern: pattern, limit: DatabaseSearchProvider.QueryConstants.maxNameResults)
        let result = try await session.simpleQuery(sql)
        return result.rows.compactMap { row in
            guard row.count >= 3, let schema = row[0], let name = row[1] else { return nil }
            let definition = row[2] ?? ""
    ...
    func searchProcedures(query: String) async throws -> [SearchSidebarResult] {
        let pattern = PostgresSearchSQL.makeLikePattern(query)
        let sql = PostgresSearchSQL.procedures(pattern: pattern, limit: DatabaseSearchProvider.QueryConstants.maxNameResults)
        let result = try await session.simpleQuery(sql)
        return result.rows.compactMap { row in
            guard row.count >= 3, let schema = row[0], let name = row[1] else { return nil }
            let definition = row[2] ?? ""
    ...
    func searchTriggers(query: String) async throws -> [SearchSidebarResult] {
        let pattern = PostgresSearchSQL.makeLikePattern(query)
        let sql = PostgresSearchSQL.triggers(pattern: pattern, limit: DatabaseSearchProvider.QueryConstants.maxNameResults)
        let result = try await session.simpleQuery(sql)
        return result.rows.compactMap { row in
    ...
    func searchColumns(query: String) async throws -> [SearchSidebarResult] {
        let pattern = PostgresSearchSQL.makeLikePattern(query)
        let sql = PostgresSearchSQL.columns(pattern: pattern, limit: DatabaseSearchProvider.QueryConstants.maxColumnResults)
        let result = try await session.simpleQuery(sql)
        return result.rows.compactMap { row in
    ...
    func searchIndexes(query: String) async throws -> [SearchSidebarResult] {
        let pattern = PostgresSearchSQL.makeLikePattern(query)
        let sql = PostgresSearchSQL.indexes(pattern: pattern, limit: DatabaseSearchProvider.QueryConstants.maxNameResults)
        let result = try await session.simpleQuery(sql)
        return result.rows.compactMap { row in
            guard row.count >= 4, let schema = row[0], let table = row[1], let name = row[2] else { return nil }
            let definition = row[3] ?? ""
    ...
    func searchForeignKeys(query: String) async throws -> [SearchSidebarResult] {
        let pattern = PostgresSearchSQL.makeLikePattern(query)
        let sql = PostgresSearchSQL.foreignKeys(pattern: pattern, limit: DatabaseSearchProvider.QueryConstants.maxNameResults)
        let result = try await session.simpleQuery(sql)
        return result.rows.compactMap { row in
            guard row.count >= 7,
                  let name = row[0],
