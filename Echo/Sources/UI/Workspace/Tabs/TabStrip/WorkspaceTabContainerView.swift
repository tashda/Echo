...
struct WorkspaceTabContainerView: View {
    ...
    private var recentConnectionItems: [RecentConnectionItem] {
        workspaceSessionStore.recentConnections.compactMap { record in
    ...
    private func createInitialTabIfNeeded() {
        guard tabStore.tabs.isEmpty,
              let activeSession = workspaceSessionStore.sessionManager.activeSession else { return }

        workspaceSessionStore.openQueryTab(for: activeSession)
    }
