...
private struct TabPreviewCard: View {
    ...
    var body: some View {
        let baseShape = RoundedRectangle(cornerRadius: 22, style: .continuous)
        ...
        .overlay(focusRing)
        .clipShape(baseShape)
        .shadow(color: cardShadow, radius: isFocused ? 14 : 8, y: isFocused ? 8 : 5)
        .onHover { hovering in
            isHovering = hovering
        }
        .onTapGesture {
    ...
    private var previewSection: some View {
        previewContent
            ...
            .padding(18)
            .frame(height: 150)
            .background(previewBackground.clipShape(RoundedRectangle(cornerRadius: 22, style: .continuous)))
    }
    ...
    private var queryMetrics: [Metric] {
        ...
        if let event = query.messages.last(where: { $0.severity != .debug }) {
            items.append(Metric(icon: "clock.arrow.circlepath", text: relativeDescription(for: event.timestamp), color: .secondary))
        }

        let rows = query.rowProgress.displayCount
        if rows > 0 {
            items.append(Metric(icon: "tablecells", text: "\(formattedNumber(rows)) rows", color: .secondary))
        }
    ...
    private var diagramMetrics: [Metric] {
        ...
        var items: [Metric] = []
        items.append(Metric(icon: "square.grid.2x2.fill", text: "\(diagram.nodes.count) node\(diagram.nodes.count == 1 ? "" : "s")", color: .secondary))
        switch diagram.loadSource {
        case .live(let date):
            items.append(Metric(icon: "clock.arrow.circlepath", text: relativeDescription(for: date), color: .secondary))
        case .cache(let date):
            items.append(Metric(icon: "archivebox.fill", text: "Cached \(relativeDescription(for: date))", color: .secondary))
        }
    ...
    private var structureMetrics: [Metric] {
        ...
        return [
            Metric(icon: "tablecells", text: "\(editor.columns.count) column\(editor.columns.count == 1 ? "" : "s")", color: .secondary),
    ...
    private var tabStatus: (icon: String, text: String, color: Color) {
        switch tab.kind {
        case .query:
            guard let query = tab.query else { return ("clock", "Not executed yet", .secondary) }
        ...
        case .diagram:
            if let diagram = tab.diagram {
                ...
                return ("chart.xyaxis.line", "Diagram ready", .secondary)
            }
        ...
        case .structure:
            if let editor = tab.structureEditor {
                ...
                return ("tablecells", "Structure ready", .secondary)
            }
    ...
    private var cardBackground: some View {
        RoundedRectangle(cornerRadius: 22, style: .continuous)
            .fill(colorScheme == .dark ? Color.white.opacity(isFocused ? 0.16 : 0.12) : Color.white)
    }
    ...
    private var cardBorder: some View {
        RoundedRectangle(cornerRadius: 22, style: .continuous)
            .stroke(
                ...
                lineWidth: isDropTarget ? 2.4 : (isFocused ? 1.4 : 1)
            )
    }

    private var focusRing: some View {
        RoundedRectangle(cornerRadius: 22, style: .continuous)
            .stroke(themeManager.accentColor.opacity(isFocused ? 0.35 : 0), lineWidth: 2.6)
    }

    private var cardShadow: Color {
        Color.black.opacity(colorScheme == .dark ? (isFocused ? 0.55 : 0.45) : (isFocused ? 0.22 : 0.12))
    }
