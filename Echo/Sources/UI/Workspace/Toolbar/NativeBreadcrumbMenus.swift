...
final class ConnectionsPopoverController: NSViewController, NSTableViewDelegate, NSTableViewDataSource, NSSearchFieldDelegate {
    ...
    init(connectionStore: ConnectionStore, workspaceSessionStore: WorkspaceSessionStore) {
...
final class DatabasePopoverController: NSViewController, NSTableViewDelegate, NSTableViewDataSource, NSSearchFieldDelegate {
    ...
    init(workspaceSessionStore: WorkspaceSessionStore, connectionID: UUID) {
        self.workspaceSessionStore = workspaceSessionStore
    ...
    private func loadDatabases() {
        ...
        loadTask = Task { [weak self] in
            ...
            guard let session = self.workspaceSessionStore.sessionManager.sessionForConnection(self.connectionID) else {
            ...
            await self.workspaceSessionStore.refreshDatabaseStructure(for: session.id, scope: .full)
    ...
    private func configureDatabase(cell: NSTableCellView, name: String) {
        ...
        if let checkmarkView = cell.viewWithTag(1) as? NSImageView {
            let isSelected = workspaceSessionStore.sessionManager.sessionForConnection(connectionID)?.selectedDatabaseName == name
            checkmarkView.alphaValue = isSelected ? 1 : 0
        }
        if let iconView = cell.viewWithTag(2) as? NSImageView {
    ...
    private func updateSelection() {
        guard let selectedName = workspaceSessionStore.sessionManager.sessionForConnection(connectionID)?.selectedDatabaseName,
    ...
    private func selectDatabase(at index: Int) {
        ...
        loadTask = Task { [weak self] in
            guard let self else { return }
            guard let session = self.workspaceSessionStore.sessionManager.sessionForConnection(self.connectionID) else { return }
            await self.workspaceSessionStore.loadSchemaForDatabase(name, connectionSession: session)
            await MainActor.run {
