...
struct BreadcrumbNavigator: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .onChange(of: workspaceSessionStore.sessionManager.sessions.count) { _, _ in
                updateBreadcrumbSegments()
            }
            .onChange(of: workspaceSessionStore.sessionManager.activeSession?.selectedDatabaseName) { _, _ in
                updateBreadcrumbSegments()
            }
    ...
    private var statusText: String? {
        ...
        let state = workspaceSessionStore.connectionStates[selectedID]
        switch state {
        case .testing:
            return "Testing…"
        case .connecting:
        ...
        case .none:
            if workspaceSessionStore.sessionManager.sessionForConnection(selectedID) != nil {
                return "Connected"
            }
            return "Disconnected"
        }
    ...
    private func menuController(for index: Int) -> NSViewController? {
        switch index {
        case 0:
            return ConnectionsPopoverController(connectionStore: connectionStore, workspaceSessionStore: workspaceSessionStore)
        case 1:
            guard let connectionID = connectionStore.selectedConnectionID else { return nil }
            return DatabasePopoverController(workspaceSessionStore: workspaceSessionStore, connectionID: connectionID)
        default:
            return nil
        }
    }
    ...
    private func updateBreadcrumbSegments() {
        ...
        if let connectionID = connectionStore.selectedConnectionID,
           let session = workspaceSessionStore.sessionManager.sessionForConnection(connectionID),
