...
struct SearchSidebarView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            viewModel.setQueryTabProvider { [weak workspaceSessionStore] in
                queryTabSnapshots(from: workspaceSessionStore)
            }
    ...
    private var activeSession: ConnectionSession? {
        if let selectedID = connectionStore.selectedConnectionID,
           let session = workspaceSessionStore.sessionManager.sessionForConnection(selectedID) {
            return session
        }
        return workspaceSessionStore.sessionManager.activeSession
    }
    ...
    private func restoreCache(for key: SearchSidebarContextKey?) {
        let cache = key.flatMap { workspaceSessionStore.searchSidebarCaches[$0] } ?? SearchSidebarCache()
    ...
    private func persistActiveCache() {
        ...
        if workspaceSessionStore.searchSidebarCaches[key] != snapshot {
            workspaceSessionStore.searchSidebarCaches[key] = snapshot
        }
    }
    ...
    private func handleResultTap(_ result: SearchSidebarResult, openInNewTab: Bool = false) {
        ...
        case .queryTab(let tabID, let connectionSessionID):
            workspaceSessionStore.sessionManager.setActiveSession(connectionSessionID)
    ...
    private func openStructure(for session: ConnectionSession, schema: String, table: String, focus: TableStructureSection?) {
        ...
        workspaceSessionStore.openStructureTab(for: session, object: object, focus: focus)
    }
    ...
    private func openDefinition(for objectName: String, schema: String, type: SchemaObjectInfo.ObjectType, in session: ConnectionSession) {
        Task {
            do {
                ...
                await MainActor.run {
                    workspaceSessionStore.openQueryTab(for: session, presetQuery: definition)
                }
            } catch {
                await MainActor.run {
