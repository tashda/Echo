        ...
        .popover(isPresented: binding(for: .connection), attachmentAnchor: .rect(.bounds), arrowEdge: .bottom) {
            ConnectionPopover(
                ...
                onSelectFolder: { folder in
                    if navigationState.selectedConnection == nil {
                ...
                onSelectConnection: { connection in
                    ...
                    let defaultDatabase = connection.database.trimmingCharacters(in: .whitespacesAndNewlines)
                    let requiresDatabaseSelection = defaultDatabase.isEmpty
                    ...
                    Task {
                        ...
                        if requiresDatabaseSelection {
                            if let session = appModel.sessionManager.sessionForConnection(connection.id),
                            ...
                            await MainActor.run {
                                if navigationState.selectedConnection?.id == connection.id {
                                    navigationState.selectedConnection = appModel.connections.first(where: { $0.id == connection.id }) ?? navigationState.selectedConnection
                                    activePopover = .database
                                }
                            }
                        } else {
                            await MainActor.run {
                                if navigationState.selectedConnection?.id == connection.id {
                                    navigationState.selectedConnection = appModel.connections.first(where: { $0.id == connection.id }) ?? navigationState.selectedConnection
                                    navigationState.selectDatabase(defaultDatabase)
                                }
                            }
                        }
                    }
