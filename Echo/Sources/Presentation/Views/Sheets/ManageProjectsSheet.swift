...
private struct ProjectAppearanceSection: View {
    ...
    @State private var localEnableAutocomplete: Bool
    ...
    init(project: Project) {
        ...
        _localEnableAutocomplete = State(initialValue: project.settings.enableAutocomplete ?? true)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                Toggle(isOn: projectEnableAutocompleteBinding) {
    ...
    private var currentPalette: SQLEditorPalette {
        ...
        return appModel.globalSettings.defaultPalette(for: tone)
    ...
    private func resetDisplayOverrides() {
        Task {
            ...
            await appModel.updateProjectAppearance(projectID: project.id) { settings in
                ...
                settings.enableAutocomplete = nil
    ...
    private func syncLocalState(with project: Project) {
        ...
        localEnableAutocomplete = project.settings.enableAutocomplete ?? resolved.autoCompletionEnabled
    ...
    private var projectEnableAutocompleteBinding: Binding<Bool> {
        Binding(
            get: { localEnableAutocomplete },
            set: { newValue in
                localEnableAutocomplete = newValue
                Task {
                    await appModel.updateProjectAppearance(projectID: project.id) { settings in
                        settings.enableAutocomplete = newValue
                    }
                }
            }
        )
