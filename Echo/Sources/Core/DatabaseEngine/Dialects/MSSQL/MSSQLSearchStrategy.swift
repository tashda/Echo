...
struct MSSQLDatabaseSearchStrategy: DatabaseSearchStrategy {
    ...
    func searchTables(query: String) async throws -> [SearchSidebarResult] {
        ...
        let sql = """
        SELECT TOP \(ObjectSearchProvider.QueryConstants.maxNameResults)
            TABLE_SCHEMA,
            TABLE_NAME
    ...
    func searchTriggers(query: String) async throws -> [SearchSidebarResult] {
        ...
        return result.rows.compactMap { row in
            ...
            let events = row[3] ?? ""
            let timing = row[4] ?? ""
            let snippet = ObjectSearchProvider.makeSnippet(from: events, matching: query)
    ...
    func searchColumns(query: String) async throws -> [SearchSidebarResult] {
        let pattern = ObjectSearchProvider.makeLikePattern(query)
        let sql = """
        SELECT TOP \(ObjectSearchProvider.QueryConstants.maxColumnResults)
            TABLE_SCHEMA,
            TABLE_NAME,
            COLUMN_NAME,
            DATA_TYPE
