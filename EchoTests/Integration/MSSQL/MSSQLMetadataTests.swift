...
final class MSSQLMetadataTests: MSSQLDockerTestCase {
    ...
    func testGetTableSchema() async throws {
        try await withTempTable(
            columns: "id INT PRIMARY KEY, name NVARCHAR(100) NOT NULL, email NVARCHAR(200), age INT"
        ) { tableName in
    ...
    func testGetTableSchemaIncludesDataTypes() async throws {
        try await withTempTable(
            columns: "id INT, amount DECIMAL(10,2), created_at DATETIME2"
        ) { tableName in
            let columns = try await session.getTableSchema(tableName, schemaName: "dbo")
    ...
    func testGetTableStructureDetailsColumns() async throws {
        try await withTempTable(
            columns: "id INT NOT NULL, name NVARCHAR(100), value DECIMAL(10,2) DEFAULT 0"
        ) { tableName in
    ...
    func testGetTableStructureDetailsPrimaryKey() async throws {
        try await withTempTable(
            columns: "id INT PRIMARY KEY, name NVARCHAR(100)"
        ) { tableName in
            let details = try await session.getTableStructureDetails(schema: "dbo", table: tableName)
    ...
    func testGetTableStructureDetailsIndexes() async throws {
        ...
        try await execute("CREATE TABLE dbo.[\(tableName)] (id INT PRIMARY KEY, name NVARCHAR(100), email NVARCHAR(200))")
    ...
    func testGetTableStructureDetailsForeignKeys() async throws {
        ...
        try await execute("CREATE TABLE dbo.[\(parentTable)] (id INT PRIMARY KEY, name NVARCHAR(100))")
        try await execute("""
