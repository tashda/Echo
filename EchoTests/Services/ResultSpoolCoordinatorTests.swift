...
final class ResultSpoolCoordinatorTests: XCTestCase {
    private var tempRoot: URL!
    private var manager: ResultSpoolCoordinator!

    override func setUp() async throws {
        tempRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("ResultSpoolCoordinatorTests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempRoot, withIntermediateDirectories: true)
        let config = ResultSpoolConfiguration.defaultConfiguration(rootDirectory: tempRoot)
        manager = ResultSpoolCoordinator(configuration: config)
    }
