...
final class TableStructureEditorViewModel: ObservableObject {
    ...
    struct IndexModel: Identifiable, Hashable {
        ...
        var isDirty: Bool {
            ...
            return original.name != name || original.columns != columns || original.isUnique != isUnique
        }
    }
    ...
    func addIndex(unique: Bool = false) {
        let baseName = unique ? "new_unique" : "new_index"
        indexes.append(
            IndexModel(
                original: nil,
                name: baseName,
                columns: columns.filter { !$0.isDeleted }.map { $0.name },
                isUnique: unique
            )
        )
    }
    ...
    private func apply(details: TableStructureDetails) {
        ...
        indexes = details.indexes.map { index in
            IndexModel(
                original: IndexModel.Snapshot(name: index.name, columns: index.columns, isUnique: index.isUnique),
                name: index.name,
    ...
    private func generateStatements() -> [String] {
        ...
        for index in indexes where index.isDeleted && !index.isNew {
            statements.append("DROP INDEX IF EXISTS \(quoteIdentifier(schemaName)).\(quoteIdentifier(index.original!.name));")
        }
        for index in indexes where !index.isDeleted {
            if index.isNew {
                let cols = index.columns.map(quoteIdentifier).joined(separator: ", ")
                var statement = "CREATE \(index.isUnique ? "UNIQUE " : "")INDEX \(quoteIdentifier(index.name)) ON \(qualifiedTable) (\(cols));"
                statements.append(statement)
            } else if index.isDirty {
                // Recreate by dropping then adding
                ...
                let cols = index.columns.map(quoteIdentifier).joined(separator: ", ")
                let stmt = "CREATE \(index.isUnique ? "UNIQUE " : "")INDEX \(quoteIdentifier(index.name)) ON \(qualifiedTable) (\(cols));"
