...
final class PostgresSession: DatabaseSession {
    ...
    func getTableStructureDetails(schema: String, table: String) async throws -> TableStructureDetails {
        ...
        // Indexes (non-primary)
        var indexes: [String: (isUnique: Bool, columns: [String])?] = [:]
        let indexSQL = """
        SELECT
        ...
        for try await (indexName, isUnique, _, column) in indexResult.decode((String, Bool, Int, String?).self) {
            var entry = indexes[indexName] ?? (isUnique, [])
            if let column {
        ...
        let indexModels: [TableStructureDetails.Index] = indexes.compactMap { name, value in
            guard let value else { return nil }
            let cols = value.columns
            return TableStructureDetails.Index(name: name, columns: cols, isUnique: value.isUnique)
...
private struct CellFormatterContext {
    ...
    func stringValue(for cell: PostgresCell) -> String? {
        ...
        let dataTypeName = "\(cell.dataType)".lowercased()

        if dataTypeName.contains("bool"),
           let value = try? cell.decode(Bool.self, context: .default) {
            return value ? "true" : "false"
        }

        if (dataTypeName.contains("int") || dataTypeName.contains("serial")),
           let value = try? cell.decode(Int64.self, context: .default) {
            return String(value)
        }

        if dataTypeName.contains("float") || dataTypeName.contains("double") {
            if let value = try? cell.decode(Double.self, context: .default) {
                return formatFloatingPoint(value)
            }
        }

        if dataTypeName.contains("numeric") || dataTypeName.contains("decimal") {
            if let decimalValue = try? cell.decode(Decimal.self, context: .default) {
                return NSDecimalNumber(decimal: decimalValue).stringValue
            }
        }

        if dataTypeName.contains("uuid"),
           let uuid = try? cell.decode(UUID.self, context: .default) {
            return uuid.uuidString
        }

        if dataTypeName == "date",
           let date = try? cell.decode(Date.self, context: .default) {
            return dateFormatter.string(from: date)
        }

        if dataTypeName.contains("timestamp"),
           let date = try? cell.decode(Date.self, context: .default) {
            return timestampFormatter.string(from: date)
        }

        if dataTypeName.contains("time"),
           let date = try? cell.decode(Date.self, context: .default) {
            return timeFormatter.string(from: date)
        }

        if dataTypeName.contains("json"),
           let string = decodeTextualValue(from: cell) {
            return string
        }

        if dataTypeName.contains("bytea"),
           var buffer = cell.bytes {
